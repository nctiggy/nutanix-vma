...
- `cmd/main.go` -- Operator binary entry point (kubebuilder default path)
...
- Install kubebuilder if not present: `curl -L -o kubebuilder https://go.kubebuilder.io/dl/latest/$(go env GOOS)/$(go env GOARCH) && chmod +x kubebuilder && sudo mv kubebuilder /usr/local/bin/`
- Run: `kubebuilder init --domain nutanix.io --repo github.com/nctiggy/nutanix-vma --project-name nutanix-vma`
- NOTE: kubebuilder v4 places the entry point at `cmd/main.go`. Use this path throughout the project (NOT `cmd/operator/main.go`). All subsequent stories that register controllers modify `cmd/main.go`.
- NOTE: kubebuilder generates ~20 scaffolding files (config/, PROJECT, etc.). These are auto-generated by a single command, not hand-written code. The 5-8 file limit applies to files Ralph writes/modifies by hand.
- Go version: 1.22
- Add `kubevirt.io/api` and `kubevirt.io/containerized-data-importer-api` to `go.mod`. Run `go mod tidy` -- this pulls a large dependency tree; it may take a minute.
- Makefile: extend the kubebuilder-generated Makefile (do NOT replace it) with additional targets: `test-integration`, `test-e2e`, `docker-push`. The generated Makefile already provides `build`, `test`, `lint`, `generate`, `manifests`, `install`, `deploy`, `docker-build`.
- Dockerfile: multi-stage build, `gcr.io/distroless/static:nonroot` base, copy operator binary from `cmd/main.go`
- AGENTS.md: use the content from Section 6 "AGENTS.md Content" of this PRD. Update the entry point path to `cmd/main.go`.
...
- `migration_types.go`: MigrationSpec (PlanRef, Cutover timestamp, Cancel []string), MigrationStatus (Phase, Started, Completed, VMs []VMMigrationStatus). VMMigrationStatus must include: ID, Name, Phase, Pipeline []PipelineStep, Error, **and resource tracking fields for idempotency**: SnapshotUUID, ImageUUIDs []string, TaskUUID, DataVolumeNames []string, OriginalPowerState. These fields allow the controller to resume after restart without creating duplicate resources.
...
**Implementation details:**
- `types.go`: Forward-declare ALL Nutanix API types as minimal structs (just the fields needed for interface signatures to compile). Full struct fields are populated in Story 4. Types needed: `VM`, `Disk`, `NIC`, `BootConfig`, `GPU`, `Reference`, `IPAddress`, `Subnet`, `StorageContainer`, `RecoveryPoint`, `Image`, `Task`, `CBTClusterInfo`, `ChangedRegions`, `ChangedRegion`, `PowerState` (string type alias).
- `client.go`: `NutanixClient` interface with ALL 20 methods explicitly:
  ```
  ListVMs, GetVM, PowerOffVM, PowerOnVM, GetVMPowerState,
  CreateRecoveryPoint, GetRecoveryPoint, DeleteRecoveryPoint,
  CreateImageFromDisk, GetImage, DownloadImage, DeleteImage,
  CloneVMFromRecoveryPoint, DeleteVM,
  DiscoverClusterForCBT, GetChangedRegions,
  ListSubnets, ListStorageContainers, ListClusters
  ```
  Plus `ClientConfig` struct, `NewClient()` factory, `httpClient` struct with stub implementations returning `errors.New("not implemented: <MethodName>")`.
...
**Acceptance Criteria:**
- `NutanixClient` interface fully defined with all 20 methods
- Forward-declared types allow interface to compile
...
- `make build` passes (no compilation errors from undefined types)
...
**Implementation details:**
- `internal/validation/validator.go`: `Validate(ctx context.Context, vm *nutanix.VM, opts ValidationOptions) []Concern`. `ValidationOptions` struct contains: NetworkMap, StorageMap, ExistingVMs (for MAC conflict check), TargetNamespace, and an optional `client.Client` (Kubernetes client for target-side validation: StorageClass, NAD, CDI/KubeVirt API group existence). If the K8s client is nil, target-side checks are skipped (allows unit testing without envtest).
...
**Implementation details:**
- `test/integration/suite_test.go`: envtest.Environment setup, register VMA CRD schemes. Start mock Nutanix server. Cleanup in AfterSuite. Install VMA CRD YAMLs from `config/crd/bases/`.
  - **KubeVirt/CDI CRDs**: For envtest, we do NOT need the full KubeVirt/CDI CRDs. The integration tests verify VMA controller behavior (creating DataVolume/VirtualMachine CRs), not CDI/KubeVirt controller behavior. Use `runtime.Scheme` to register KubeVirt and CDI types from their Go API packages (`kubevirt.io/api`, `kubevirt.io/containerized-data-importer-api`). Create minimal CRD YAML stubs in `test/fixtures/crds/` containing only the fields envtest needs for API registration. Generate these stubs using: `controller-gen crd paths=./vendor/kubevirt.io/api/core/v1/... output:dir=test/fixtures/crds/` or hand-write minimal CRDs with just `apiVersion`, `kind`, `metadata.name`, `spec.versions[0].schema` containing a permissive `x-kubernetes-preserve-unknown-fields: true`.
  - Add a Makefile target `test-fixtures` that creates these CRD stubs.
- `internal/controller/provider_controller.go`: Reconcile NutanixProvider CRs. Create Nutanix client from spec + Secret (using `NewClient()`). Fetch inventory: call `ListVMs()`, `ListSubnets()`, `ListClusters()`, then for each discovered PE cluster call `ListStorageContainers(peURL)`. Update status with counts and cluster list. Set conditions. Requeue after RefreshInterval. Add finalizer.
...
**Implementation details:**
- `internal/controller/plan_controller.go`: Reconcile MigrationPlan CRs. Resolve Provider reference -> read Provider's Secret -> create a Nutanix client (`nutanix.NewClient()`) -> call `GetVM(uuid)` for each VM in the plan to get full VM details. Resolve NetworkMap and StorageMap references. Run validation engine against each VM with the maps. Update per-VM status with concerns. Set plan phase.
  - NOTE: The Plan controller creates its OWN Nutanix client from the Provider's credentials (same pattern as the Provider controller). It does NOT read VM details from Provider status -- the Provider status only has counts, not full VM specs.
...
- Replace ExportDisks stub: call `client.CreateImageFromDisk()` for each disk. The fallback export path (clone VM from recovery point -> create image from clone -> delete clone) is deferred to a separate follow-up story to keep this story's scope manageable. For now, if `CreateImageFromDisk()` fails, the phase fails with an error. The fallback will be added once real cluster testing confirms whether direct image creation works.
